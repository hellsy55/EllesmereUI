...
UpdateCastBar = function(dt)
    ...
    if castBarFrame._cancelDisplayUntil then
        if GetTime() >= castBarFrame._cancelDisplayUntil then
...
OnCastStart = function()
    ...
    -- A new cast immediately replaces a pending cancelled-cast message. Only
    -- rebuild (restoring the user's normal fill/gradient/text) when a cancel
    -- display actually overrode them -- BuildCastBar is not part of the
    -- normal per-cast path here, so this stays a rare-path cost.
    if castBarFrame._cancelDisplayUntil then
        castBarFrame._cancelDisplayUntil = nil
        BuildCastBar()
    end
...
-- GCD Bar. Uses the same detection logic as the cursor GCD Circle.
BuildGCDBar = function()
        ...
        gcdBarFrame:SetScript("OnEvent", function(self, event, unit, _, spellID)
            ...
            if event == "UNIT_SPELLCAST_START" or event == "UNIT_SPELLCAST_CHANNEL_START"
               or event == "UNIT_SPELLCAST_EMPOWER_START" then
                -- Remember this spell had a cast time/channel/empower so the succeeded
                -- it fires can be skipped under instant-only. Channels/empowers fire
                -- succeeded on start. verify there's an actual cast time, a spell made
                -- instant (e.g. Swiftness Regrowth) will count as instant cast
    ...
    if g.alwaysShow and not (g.instanceOnly and not IsInInstance()) then
        bar:SetValue(g.idleShowFill and 1 or 0)
