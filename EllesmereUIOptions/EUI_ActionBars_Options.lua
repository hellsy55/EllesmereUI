    ...
    local function BuildSharedBarSettings(parent, y)
        ...
        do
            ...
            do
                ...
                -- Show During Drag is offered only while THIS bar's visibility is Never (other modes
                -- already surface during a drag); two prebuilt popups, cog opens the one matching the
                -- current mode, so the row appears/disappears live with no page rebuild.
                ...
                local _, visCogShow = EllesmereUI.BuildCogPopup({
                    title = "Visibility",
                    rows = { MORow(), SkyridingRow() },
                })
                local _, visCogShowNever = EllesmereUI.BuildCogPopup({
                    title = "Visibility",
                    rows = { MORow(), DragRow(), SkyridingRow() },
                })
