...
-- oldest) entry per name wins. Alphabetical, case-insensitive by name (by request:
-- editable filters should always list alphabetically) -- applied to both the Filters
-- assignment dropdown (via DedupedFilterItems below) and the Filter Editor's own
-- sidebar (ns.PABMP_ShowFilterEditor). Always returns a FRESH copy -- ns.PAB_Filters()
-- hands back the live persisted list (EllesmereUIUnitFrames_PlayerAuraBars.lua's
-- ns.PAB_Filters, `store.list` directly), so sorting in place would silently reorder
-- SavedVariables every time the editor is opened.
