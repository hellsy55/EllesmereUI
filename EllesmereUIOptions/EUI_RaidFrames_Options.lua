...
initFrame:SetScript("OnEvent", function(self)
    ...
    local function BuildVisualSections(parent, y, W, onSection)
        ...
        -------------------------------------------------------------------
        --  TARGETED SPELLS (raid only; party page builds its own section on the ts* keys).
        --  tsRaid* keys sit deliberately OUTSIDE the party section-sync system, hence no onSection call.
        --  SECTION RETIRED at 12.1: the game no longer provides the targeted-spells
        --  data this fed, so the settings are hidden (a visible section would look
        --  broken). Runtime module left in place and inert; stored ts*/tsRaid* keys
        --  go unread. Drop the leading `false and` to revive.
        -------------------------------------------------------------------
        if false and not _partyCtx then
            local tsHeader
            ...
            local _, hmCogShow = EllesmereUI.BuildCogPopup({
                ...
                rows = {
                    ...
                    { type="toggle", label="Show Names in Raid",
                      ...
                      set=function(v) HMS().showNames = v and nil or false; HMRefresh() end },
                    { type="toggle", label="Class Colored Names",
                      get=function() return HMS().classNames ~= false end,
                      set=function(v) HMS().classNames = v and nil or false; HMRefresh() end },
                },
            })
    ...
    local function BuildPartyPage(pageName, parent, yOffset)
        ...
        -------------------------------------------------------------------
        --  TARGETED SPELLS (party; the raid Frames tab builds its own section
        --  on independent tsRaid* keys)
        --  SECTION RETIRED at 12.1: same reason as the raid section above --
        --  the game no longer provides the data. Change `if false` back to
        --  `do` (or `if true`) to revive.
        -------------------------------------------------------------------
        if false then
            local tsHeader
            tsHeader, h = W:SectionHeader(parent, "TARGETED SPELLS", y); y = y - h
