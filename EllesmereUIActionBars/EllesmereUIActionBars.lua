...
ns._EABZeroCountAlpha = function(fd, fs, v, action)
    ...
    if action then
        ...
        if mc ~= nil and not (issecretvalue and issecretvalue(mc)) and mc > 1 then
            ...
            if cc == nil then
                -- Field is non-nilable by contract; belt-and-suspenders so a
                -- missing read can't strand a hidden count.
...
-- Auto-Compact is available on every standard action bar (MainBar, Bar2-10).
-- Stance/Pet bars reuse Blizzard buttons and have their own dynamic-count
-- rules, and visibility-only bars have no button grid at all. Kept as a
-- function (not a hardcoded check at every call site) so it stays a single,
...
function EAB.VisibilityCompat.Copy(dst, src, dstNoGroupModes)
    ...
    -- Show During Drag travels with the copy (inert unless target mode is Never).
...
local function GetOrCreateButton(slot, parent, info, index, skipProtected)
    ...
    if info.isStance then
        ...
        -- Pickup modifier (shift-click to move abilities): temporarily disable
        -- useOnKeyDown so the action doesn't fire on mouse-down and the drag consumes
        -- it. MOUSE-ONLY via IsUnderMouse: keybinds also arrive as OnClick
        -- (SetOverrideBindingClick), and a modified keybind press must cast normally. A
        -- wrap's post-body only runs when the pre-body returns a message -- flipping
        -- without one left useOnKeyDown stuck false (cast-on-release, delayed presses,
        -- GCD swipes persisting until an out-of-combat rebuild). Pre returns a message
        -- only when it flips; post restores right after the down-click handler, so a
        -- mouse pickup's up-click simply doesn't cast (up-clicks never fire in key-down
        -- mode) -- the intended behavior.
...
function EAB_VTABLE.ForceButtonRefresh(btn, action)
    ...
    if icon then
        ...
        -- nothing until hover runs Blizzard's Update. Gate on HasAction (a
        -- clean boolean), never on the texture value, which can be secret.
...
do
    ...
    -- refire and other UpdateKeybinds callers are load-time/rare, so an in-combat slot
    -- change would leave an empowered spell on its native binding with no
    -- pressAndHoldAction/typerelease (Empowered Spell Input flips Hold-and-Release to
    -- Press-and-Tap) until something unrelated rebuilds. Defer to PLAYER_REGEN_ENABLED,
    ...
    function EAB:SetupEventDispatcher()
        ...
        -- Desaturation curves: secret-safe duration -> 0/1 via
        -- EvaluateRemainingDuration, so secret cooldown/charge numbers are
        -- never compared directly.
        --   desatCurveAny  : 1 for any active cooldown (GCD filtered via isOnGCD)
        --   desatCurveReal : 1 only when cooldown is longer than the GCD. Used
        --                    for charge spells -- a banked charge shows only a
        --                    GCD-length main cooldown (stays colored); at 0
        --                    charges the longer recharge drives it (desaturates).
        ...
        -- On-CD alpha curve for charge spells/items: keyed on TOTAL duration
        -- like desatCurveReal (GCD-length = full alpha, real cooldown = the
        -- user's dim value). Rebuilt only when the alpha setting changes.
        ...
        -- API call when both features are off. TOTAL duration for charge spells/items,
        -- never REMAINING: writes are static, and evaluating the 1.6s "real cooldown"
        -- step against remaining reads 0 for any cooldown event landing in the final
        -- 1.6s (every cast in combat), re-saturating early. The total never decays, so
        -- the classification holds for the cooldown's whole life; OnCooldownDone
        -- restores the icon the moment it actually completes.
        local function RefreshCooldownVisuals(btn, action, cdInfo, durObj, chargeInfo)
            ...
            if desatOn then
                local val = 0
                if active then
                    -- A GCD must never classify a charge spell/item as on cooldown: the
                    -- 1.6s step tests the TOTAL duration of whatever object the engine
                    -- hands back, and for a charge spell that's the RECHARGE PERIOD
                    -- (e.g. 20s) even while charges are banked -- every GCD would grey
                    -- a spell at FULL charges. isOnGCD separates the cases cleanly
                    -- (measured: 0 charges -> isActive=true isOnGCD=false
                    -- dur=27; banked charge mid-GCD -> isOnGCD=true dur=1.05).
                    -- One hoisted guard for both classes: a GCD is never a cooldown here.
                    if not cdInfo.isOnGCD then
                        if useRealCurve then
                            ...
                            elseif durObj.EvaluateRemainingDuration and desatCurveReal then
                                -- No total evaluator: remaining is start-accurate, Done edge fixes the tail.
            ...
            if alphaOn then
                if active then
                    if useRealCurve and not cdInfo.isOnGCD and durObj.EvaluateTotalDuration then
                        -- Same total-duration classification + isOnGCD guard
                        -- as desat: total is the RECHARGE PERIOD for a charge
                        -- spell, so the 1.6s step alone doesn't filter the GCD.
                    ...
                    elseif icon.SetAlphaFromBoolean and durObj.IsZero
                       and not cdInfo.isOnGCD then
                        -- IsZero() is a secret boolean; SetAlphaFromBoolean consumes it with no Lua comparison.
        ...
        -- mixin path. The memo is write-behind everywhere else on purpose:
        -- paths that paint the icon without stamping it (ForceButtonRefresh,
        -- infrequent walk's UpdateAction) always paint the CURRENT texture,
        -- so a stale memo can only cause one redundant repaint, never a wrong
        -- skip. Used by the full walk below and the payload-targeted
...
-- Resolve the effective icon count for a bar (override, or its default count).
function EAB_VTABLE.GetConfiguredNumIcons(key, info, s)
...
function EAB_VTABLE.GetEffectiveShowEmpty(key, info, s)
    -- Auto-Compact owns empty-slot visibility while it's actively packing
    -- the bar, without mutating the user's saved Always Show Buttons
    -- preference. No-op for bars whose toggle is off.
...
function EAB_VTABLE.ResolveActionSlot(key, info, btn, index)
...
function EAB_VTABLE.ButtonHasAction(key, info, btn, index)
    if not btn then return false end
    if info.isStance or info.isPetBar then
        return ButtonHasAction(btn, info.blizzBtnPrefix)
    end
    local slot = EAB_VTABLE.ResolveActionSlot(key, info, btn, index)
    return slot ~= nil and HasAction(slot) or false
end

-- Auto-Compact. Kept on the existing vtable to avoid consuming any of this
-- chunk's already-tight Lua 5.1 local-variable budget.
function EAB_VTABLE.Compact.IsEnabled(key, info, s)
    return s and s.autoCompactSlots == true
        and EAB:BarSupportsAutoCompact(key)
end

-- Item/toy/battlepet drags aren't part of the base spell/macro drag set the
-- general grid system reacts to, but should still expand a compacted bar so
-- there's somewhere to drop them.
...
function EAB_VTABLE.Compact.GetOccupancy(key, info, s, buttons, numIcons)
    ...
    for i = 1, numIcons do
        if EAB_VTABLE.ButtonHasAction(key, info, buttons[i], i) then
            mask = mask + bitValue
            count = count + 1
        end
        bitValue = bitValue * 2
...
-- Shared by unlock-mode resize handles (setWidth/setHeight) so dragging the
-- resize handle while actively compacting reverse-engineers button size from
-- the packed slot count instead of the configured one. Falls back to
-- numIcons unchanged when Auto-Compact is off, expanded, or buttons aren't
-- available yet.
...
local function ComputeBarLayout(key)
    ...
    local numIcons = EAB_VTABLE.GetConfiguredNumIcons(key, info, s)
    ...
    local showEmpty = EAB_VTABLE.GetEffectiveShowEmpty(key, info, s)
...
local function LayoutBar(key)
    ...
    do
        ...
        -- Auto-Compact's on/off, expanded/packed, and occupied-slot state are
        -- outcome-changing inputs the stamp must cover too -- without this,
        -- toggling the setting or crossing the skyriding edge would compute
        -- an identical stamp to the prior pass and silently no-op.
        local compactSig = ""
        if EAB:BarSupportsAutoCompact(key) then
            local cEnabled = EAB_VTABLE.Compact.IsEnabled(key, info, s)
            local cExpanded = EAB_VTABLE.Compact.IsExpanded(key, info, s)
            local cMask = 0
            if cEnabled then
                cMask = EAB_VTABLE.Compact.GetOccupancy(key, info, s, buttons, nIcoS)
            end
            compactSig = (cEnabled and "1" or "0") .. (cExpanded and "1" or "0") .. tostring(cMask)
        end
    ...
    -- Show empty slots (stance bar always forces this off; Auto-Compact
    -- forces this off too while it's actively packing the bar)
    local showEmpty = EAB_VTABLE.GetEffectiveShowEmpty(key, info, s)
        ...
        if self:BarSupportsAutoCompact(key) then
            ...
            if s and frame and buttons then
                ...
                if force
                    ...
                    or fd.autoCompactNumIcons ~= numIcons then
                    LayoutBar(key)
...
function EAB:ApplyAlwaysShowButtons(barKey)
    ...
    local buttons = barButtons[barKey]
    if not buttons then return end
    local showEmpty = EAB_VTABLE.GetEffectiveShowEmpty(barKey, info, s)
    ...
    local function CanEnter(self)
        ...
        if s then
            ...
            local showEmpty = info
                and EAB_VTABLE.GetEffectiveShowEmpty(barKey, info, s)
                or true
...
-------------------------------------------------------------------------------
--  "Always Show While Skyriding" (per-bar visShowSkyriding)
--
--  [advflyable,flying] is the exact macro conditional the secure state
--  driver already uses for the "Show while Dragonriding" visibility mode
--  (see BuildVisibilityString). It re-evaluates natively -- even in combat
--  -- so the driver-side override below needs no Lua help.
--
--  The mouseover ALPHA fade, however, is Lua-driven (SetAlpha calls in
--  RefreshMouseover / Hover.FadeOut) and is not tied to the state driver at
--  all, so it needs its own airborne-edge check here plus a live poke when
--  gliding starts/stops.
-------------------------------------------------------------------------------
do
    local function IsSkyriding()
        return (IsAdvancedFlyableArea and IsAdvancedFlyableArea() and IsFlying and IsFlying()) and true or false
    end
    ...
    if EllesmereUI._hasGlidingEvent then
        skyEvt:RegisterEvent("PLAYER_IS_GLIDING_CHANGED")
    end
    ...
    skyEvt:SetScript("OnEvent", function(_, event)
        ...
        if event == "PLAYER_ENTERING_WORLD" then
            -- Zone transitions (instance entry/exit) can change mount and
            -- bonusbar state before GetBonusBarOffset()/IsFlying() settle on
            -- the same frame this event fires. Re-check one tick later too.
            C_Timer_After(0, SkyEvtRefresh)
        end
    end)
end

-- "Always Show While Skyriding" (visShowSkyriding) on Action Bar 1: while the
-- skyriding bonus action bar (bonusbar 5) is showing, switch Bar 1's
-- effective visibility to "always" -- as if the Visibility dropdown itself
-- were set to Always -- without touching the saved barVisibility/mouseover
-- settings. Reuses the same runtime-only _visOverride slot the Toggle Action
-- Bar keybind and Show During Drag features already use, so nothing here is
-- ever persisted and it plays nicely with those (never clobbers an override
...
function EAB:UpdateSkyridingVisOverride()
    ...
    if active then
        ...
        if EAB._visOverride["MainBar"] ~= "always" then
            EAB._visOverride["MainBar"] = "always"
            self:RefreshRuntimeVisibility()
        end
    elseif EAB._skyridingVisForced then
        EAB._skyridingVisForced = nil
        if EAB._visOverride and EAB._visOverride["MainBar"] == "always" then
            EAB._visOverride["MainBar"] = nil
            self:RefreshRuntimeVisibility()
        end
    end
...
-------------------------------------------------------------------------------
--  Keybind System: hybrid routing. Empower/flyout slots use
--  SetOverrideBindingClick so our buttons' pressAndHoldAction/typerelease
--  handle hold-and-release. All other slots use SetOverrideBinding to native
--  commands (ACTIONBUTTON1 etc.) so the engine handles press-and-hold repeat
--  casting natively.
-------------------------------------------------------------------------------
...
local function UpdateKeybinds()
    ...
    -- -- empower slots stuck on native bindings, where pressAndHoldAction
    -- never engages and the spell behaves as Press-and-Tap with the CVar untouched.
        ...
        if prefix and btns then
            ...
            for i, btn in ipairs(btns) do
                if btn then
                    local cmd = prefix .. i
                    local k1, k2 = GetBindingKey(cmd)
                    -- Empower spells need SetOverrideBindingClick so our button's
                    -- pressAndHoldAction/typerelease handle hold-and-release.
                    -- Non-empower spells use native SetOverrideBinding for
                    -- press-and-hold repeat casting -- EXCEPT on custom-paged
                    -- bars (above), which must click-route to track the page.
                    local slot = btn:GetAttribute("action")
                    ...
                    -- flyout separately from useClick: on a custom-paged bar
                    ...
                    -- Page-proof routing: also consult the button's BASE slot. A click
                    -- binding tracks the BUTTON, whose action attr re-pages SECURELY
                    -- in combat, so a click-routed key survives any page swap with no
                    -- rebind. Deriving isPH from the CURRENT page only meant mounting
                    -- (skyriding page) rebuilt empower keys to native, and dismounting
                    -- INTO combat blocked the rebuild back, leaving empowered spells
                    -- on Press-and-Tap for the fight. Click-routing when EITHER slot
                    -- is empower-capable removes the rebind; cost is press-and-hold
                    -- repeat while paged away from the empower slot. Guarded on the
                    -- offsets table so Pet/Stance buttons (action attr nil) never
                    -- alias into MainBar slots.
                    if not isPH then
...
function EAB:FinishSetup()
    ...
    self:RegisterEvent("CURSOR_CHANGED", function()
        ...
        elseif EAB_VTABLE.Compact.IsSupportedDragCursor(cursorType) then
            -- Compact-only cursor types (item/toy/battlepet/etc.) don't
            -- surface the classic mouseover/Never-bar behavior, but still
            -- need the grid held open so newly-expanded slots stay visible.
            if not _gridState.shown then
                OnGridChange()
            end
        else
