    ...
    if fd.cooldown then
        ...
        if not fd._swipeColorHooked then
            ...
            local function ReArmChargeRecharge()
                ...
                -- Only while a recharge genuinely runs. isActive is clean;
                -- currentCharges (secret) never read.
                ...
                cd:SetCooldownFromDurationObject(durObj)
                ApplyCdmChargeStyle(frame, cd)
                -- The display is now the REAL recharge, never a GCD, so
                -- Suppress-GCD's alpha-0 swipe (set while isOnGCD, e.g. right
                -- after pressing ANOTHER ability) must not stick or the recharge
                -- stays invisible until the active state ends. Restore the black
                -- hide-active swipe unconditionally (no-op when Suppress-GCD off).
        ...
        if fd.tex and not fd._desatOverrideHooked then
            ...
            local function onDesatChange()
                ...
                -- icon. currentCharges is SECRET in this tainted hook (cannot be
                -- compared), so use Blizzard's clean isOnActualCooldown: false =
                -- at least one charge usable, stay saturated until genuinely out.
                -- The charge-SPELL test is static charge data, NOT
                -- frame:HasVisualDataSource_Charges(): that flips FALSE while a
                -- GCD swipe is layered on top and is absent (nil) on every CDM
                -- frame, so gating on it made this branch dead code. maxCharges is
                -- stable, present and clean -- the same signal the swipe guard,
                -- Max Stacks Glow and Hide CD Text use.
...
-- Re-assert the BASE (non-active) swipe direction on a preset/custom/trinket
-- cooldown widget. Zero cost unless the reverse-swipe family of flags is
-- armed. Centralizes the GetEffectiveCustomActiveState lookup used at every
-- fresh-cooldown-push site (item swap, trinket cooldown push, racial/preset/
-- spell cooldown push) so those call sites can't drift out of sync with each
-- other; key is whatever ns.GetEffectiveCustomActiveState expects for that
-- icon (negative slotID for trinkets/presets, positive spellID for spells).
