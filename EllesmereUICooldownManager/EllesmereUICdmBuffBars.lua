...
--  spell's remaining cooldown; Charge Hash Lines instead fill one section per
--  recovered charge while the timer stays the NEXT charge's cooldown. Stacks text =
--  current charges. Ready (off cooldown/GCD-only/max charges/unknown) counts as
--  INACTIVE for hideWhenInactive; a shown-but-ready bar renders full with no timer.
--
--  FILL SOURCE ORDER:
...
local function _UpdateCooldownBar(bar, cfg)
    ...
    if sb then
        ...
        elseif durObj and sb.SetTimerDuration and timerDir then
            _restoreTBBNormalFill(bar, cfg)
            -- Engine-driven drain: the handle tracks CDR and resets live with no numbers ever
            -- read. Re-set the bar timer only when a fresh handle was fetched (event/revalidate)
            -- or the bar just appeared; the engine animates in between.
        ...
        elseif remaining then
            ...
            _restoreTBBNormalFill(bar, cfg)
            -- Ready (kept on screen) or unreadable fail-open: full bar. Plain SetValue also cancels any running bar timer.
            sb:SetMinMaxValues(0, 1)
            sb:SetValue(1)
