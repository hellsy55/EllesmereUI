...
LayoutCDMBar = function(barKey)
    ...
    -- the whole feature. The flag is only ever set by the cd-state evaluators (ns.SetCdStateShiftHidden);
    -- bars without it pay one field read per icon and never build the filtered table. Skipped frames keep their last point at alpha 0.
    do
        local filtered
        for i = 1, #icons do
...
--  Built by scanning all action bar slots; stored as { [spellID] = "formatted key" } so icon
--  display is just a lookup. Resolves binding keys per action slot (main bar paged via the EAB
--  bar's actionpage attribute, else client page APIs). Read-only + text writes on our own frames, so it is safe to run in combat (debounced upstream).
-------------------------------------------------------------------------------

-- Action bar slot -> binding name map. Non-bar-1 entries listed first so that if a spell appears on multiple bars, the more specific bar wins over bar 1.
...
-- Store a keybind under a cache key with macro-deprioritization: a direct (non-macro) bind always
-- wins. If nothing is cached yet we store it; if a macro bind was stored earlier and this one is a
-- direct spell bind, it overrides the macro. Macro binds never overwrite an existing entry. This lets a user who has both a macro and the real spell bound see the real spell's key.
        ...
        for i = 1, 12 do
            ...
            if key then
                ...
                if def.prefix == "ACTIONBUTTON" then
                    -- Main bar pages with forms/vehicles. Prefer the EAB main bar's actionpage
                    -- attribute (set by its secure page handler, covers override/vehicle pages too).
                    -- Without it (Action Bars module disabled), derive the page from the client: bonus bars (forms) map to pages 7+, else the manually selected page.
                ...
                elseif slotType == "macro" then
                    ...
                    if subType == "spell" then
                        -- "Smart" single-spell macro: Blizzard already resolved it, and `id` here IS the spellID, not a macro index -- passing it to GetMacroSpell would look up the wrong thing.
                        spellID = id
                    else
                        -- Everything else (single-item, mount/companion, multi-line/conditional...):
                        -- `id` from GetActionInfo is NOT a reliable identifier here. Resolve the real macro index via its name instead (same workaround EllesmereUICdmHooks.lua's SlotSpellID already uses).
                        local macroName = GetActionText(slot)
                        local macroIndex = macroName and GetMacroIndexByName(macroName)
                        if macroIndex and macroIndex > 0 then
                            spellID = GetMacroSpell(macroIndex)
                            if not spellID then
                                -- Not a spell-shaped macro: pull the first /use target out of the macro body and resolve it as an item instead.
                                local body = GetMacroBody and GetMacroBody(macroIndex)
                                local target = body and body:match("/use!?%s+([^\r\n]+)")
                                if target then
                                    target = target:gsub("^%[.-%]%s*", ""):match("^%s*(.-)%s*$")
                                    -- "item:NNNN" is macro-only shorthand for a direct itemID and NOT valid
                                    -- GetItemInfoInstant input (that wants a bare itemID, item name, or full link) -- extract the numeric ID ourselves.
                ...
                elseif slotType == "item" and id then
                    -- Store under negated itemID (-id) to match the FC convention for item presets/trinkets.
