...
local function _ApplyAlpha(alpha)
    ...
    -- for the border/separator/underline hosts. min() with _tabFadeAlpha so
    ...
    -- sidebar mouseover fade just below.
...
initFrame:SetScript("OnEvent", function(self)
    ...
    do
        ...
        -- MERGED (new-features + main): both temp-window paths kept on
        -- purpose. QueueFullPass (main) is the coalesced safety net --
        -- SkinPass + SyncChatFrameState + PositionChatPanelsNow -- and the
        -- immediate SkinPass/QueueTabPass pair (new-features) is kept for a
        -- snappier first paint on the temp window itself.
    ...
    --
    -- !! TAINT-RISK, KEPT ON REQUEST (merge decision) !!
    -- main removed this hooksecurefunc entirely: permanent windows already
    -- fire UPDATE_CHAT_WINDOWS, which QueueFullPass listens for above, so
    -- this block is functionally redundant with that path. It was reinstated
    -- here at the user's request to merge new-features's version rather than
    -- drop it. FCF_OpenNewWindow is reached from the same tab-menu chain as
    -- FCF_Close/FCF_PopInWindow, which this file elsewhere documents as
    -- taint-prone (see the FCF_OpenTemporaryWindow note above). If chat
    -- taint errors resurface (e.g. HistoryKeeper errors, blocked whisper
    -- actions in combat), remove this block first -- QueueFullPass alone
    -- already covers the permanent-window case safely.
    ...
    -- Permanent chat windows are ALSO covered by the full pass on
    -- UPDATE_CHAT_WINDOWS (redundant safety net; see the risk note above for
    -- why the direct hook was kept anyway).
