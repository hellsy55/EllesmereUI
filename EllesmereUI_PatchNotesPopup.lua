...
local function ShowPatchNotesPopup()
    ...
    local Finish
    local dimmer, popup = EllesmereUI.BuildPopupShell("EUIPatchNotesIntro", {
    ...
    local viewBtn = EllesmereUI.MakeActionButton(popup, FONT, "View Patch Notes", EG.r, EG.g, EG.b, { w = BTN_W })
    PP.Point(viewBtn, "BOTTOMRIGHT", popup, "BOTTOM", -BTN_GAP / 2, 40)
    viewBtn:SetScript("OnClick", function() Finish(true) end)

    local laterBtn = EllesmereUI.MakeActionButton(popup, FONT, "Maybe Later", 1, 1, 1, { w = BTN_W, secondary = true, hoverA = 0.6 })
