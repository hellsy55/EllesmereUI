...
-- Merges duplicate non-gear items by itemLink in an already-ordered list (link encodes stats,
-- so differing stats stay separate). Must run AFTER ApplySavedOrder so the first in visual order wins. Returns a new list; only winners get _mergedCount.
local function MergeDuplicates(items)
    -- Clear stale _mergedCount: one data table can be merged in several sections (category + pinned/recent) within the same refresh.
