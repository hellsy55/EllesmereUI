...
ns.BlockFactories.spec = function(blockCfg, slot, content, barCtx)
    ...
    local function OpenLoadoutSubnav()
        ...
        -- Flyout anchoring: flush against the spec popup's edge; flips to the left side when the screen runs out (clamped either way).
...
local function MMOpenWhisper(charName, bnetName)
    ...
    -- while /euidev is on -- it forces addonChallengeModeRestrictionsForced, the same
    -- secret-value restricted environment as a real Mythic+, which InProtectedInstance()
    -- does NOT see, hence the separate dev-mode check.
    local blocked
