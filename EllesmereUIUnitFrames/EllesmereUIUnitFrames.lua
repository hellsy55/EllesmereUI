...
local function ApplyUnitFrameCastColor(castbar)
    ...
    castbar:SetStatusBarColor(r, g, b, castbar._fillOp or 1)
...
function InitializeFrames()
    ...
    --  force-hides anything left over. It also does not rely on the Out of
    --  Range Alpha feature's OnShow/OnHide mirror (see CreateCastBar, where
    --  the boss Castbar's parent is reparented off the boss frame so it
    --  doesn't inherit that fade) -- it hides the Castbar's own parent
    --  directly, so it still works even if that mirror hook misses a beat.
    ---------------------------------------------------------------------------
