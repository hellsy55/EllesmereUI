...
--
--  Spell IDs extracted programmatically from EUI_RaidFrames_BuffManager2.lua's
--  PRESET_FILTERS + DEFAULT_FILTER_SPELLS (primary ids plus their `alts` flattened
--  into one list each: PAB Filters have no primary/alt grouping, every id is its own
--  checkbox row). `disabled` entries are imported too, and start unchecked here too.
-------------------------------------------------------------------------------
...
-- set. Extracted from the same PRESET_FILTERS/DEFAULT_FILTER_SPELLS source as
-- BM2_FILTER_SEED below (254 unique entries; "ALL"-class spells omitted, no header).
