...
local function CreateAbsorbBar(button, healthBar)
    ...
    local function ReanchorAbsorbToFill()
        ...
        if absorbMode == "right" or absorbMode == "left" then
            ...
            -- Overlay: curClip already clipped to the fill above. Restore the backfill's default right-anchored reverse fill.
            backfillBar:SetReverseFill(true)
            backfillBar:ClearAllPoints()
            backfillBar:SetPoint("TOPRIGHT", healthBar, "TOPRIGHT", 0, 0)
            backfillBar:SetPoint("BOTTOMRIGHT", healthBar, "BOTTOMRIGHT", 0, 0)
...
local function UpdateAbsorb(button, unit)
    ...
    if ab._sGen ~= ns._absorbGen then
        ...
        -- Show Overshield (default ON): the absorb exceeding empty health, backfilling over current
        -- health -- drawn by the backfill bar (ab) in overlay + Default-Blizz modes. OFF feeds the
        -- backfill 0 so only empty health fills; the forward bar (fw, clipped to missing health)
        -- still caps at the health-bar right edge. Right/left edge modes draw the WHOLE absorb
        -- through ab (fw hidden below) and are untouched -- overshield is meaningless there.
