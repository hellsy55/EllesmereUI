...
local function AnchorBmChainContainer(container, health, members, iscale)
    ...
    -- so geometry changes re-drive group layouts. One layout per member group:
    ...
    for j = 1, #members do
        ...
        -- Max Icons: cap each member's group below its own spell count.
...
local function BmAcquireChain(button, d, health, ch, iscale, counters)
    ...
    if entry == nil then
        ...
        if shell then
            ...
            entry = { container = shell, groups = #members }
            pool[poolKey] = entry
        else
            ...
            AK.QueueBuildJob(function()
                ...
                pool[poolKey] = { container = cc, groups = #members }
    ...
    for j = 1, #members do
        local gk = (j == 1) and "chain" or ("chain" .. j)
        cc:SetAuraGroupCandidateFilters(gk, BuildBmCand(members[j].ind, members[j].spells))
        anchorMembers[j] = { ind = members[j].ind, count = #members[j].spells }
    end
