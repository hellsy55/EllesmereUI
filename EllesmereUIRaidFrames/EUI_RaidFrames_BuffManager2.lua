...
-- Curated spell lists per preset filter: [primaryID] = { class =
-- "CLASSFILE"|"ALL", disabled = true|nil, alts = { id, ... } }. Enabled by
-- default unless `disabled`. The PRIMARY id is the checkbox row; alternates
-- (talent/rank variants) ride the engine include maps at slot construction
-- (BmIncludeMap), follow the primary's state, and never appear separately.
-- EnsureFilters() merges NEW ids additively, never overwriting user choices.
